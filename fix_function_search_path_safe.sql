-- Fix function search path warnings safely by dropping and recreating functions
-- This handles functions that already exist with different signatures

-- Drop existing functions first to avoid signature conflicts
DROP FUNCTION IF EXISTS public.user_property_access(uuid, uuid);
DROP FUNCTION IF EXISTS public.user_organization_properties(uuid);
DROP FUNCTION IF EXISTS public.get_user_role(uuid);
DROP FUNCTION IF EXISTS public.update_updated_at_column();
DROP FUNCTION IF EXISTS public.create_or_find_organization(text);
DROP FUNCTION IF EXISTS public.is_admin_user(uuid);
DROP FUNCTION IF EXISTS public.get_property_kb_documents(uuid);
DROP FUNCTION IF EXISTS public.update_document_sync_status(uuid, text);
DROP FUNCTION IF EXISTS public.create_generation_job(uuid, text, jsonb);
...
DROP FUNCTION IF EXISTS public.get_user_admin_permissions(uuid);
